...
def minmax_Traj(xData, yData):
    ...
    for a in range(length):
        c = minmaxradiuscircle.make_circle(xData[:, a], yData[:, a], numData)
        xMinmaxTraj[a] = c[0]
        yMinmaxTraj[a] = c[1]
...
def percent_minmax_Traj(xData, yData, percent):
    ...
    for a in range(length):
        ...
        c = minmaxradiuscircle.make_circle(newfixedTimeData[0:validNumData, 0], newfixedTimeData[0:validNumData, 1], validNumData)
