...
def make_circle(xCoord, yCoord, numData):
...
def percent_make_circle(xCoord, yCoord, tempNumData, percent, newX, newY):
    ...
    #present one
    c1 = make_circle(points[0:numData, 0], points[0:numData, 1], numData)
    #print(numData)
...
# compute the circumscribed circle using three given points
...
MACHINE_EPSILON = 1 + 1e-14

def point_in_circle(c, p):
    if c is not None:
        if math.sqrt(math.pow(p[0] - c[0],2)+ math.pow(p[1] - c[1],2)) <= (c[2] * MACHINE_EPSILON):
            return True
    else:
        return False
...
def center_of_circle(xData, yData, numData):
    c = make_circle(xData, yData, numData)
    return c[2]